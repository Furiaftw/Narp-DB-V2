...
/* ============================================================================
   ⚠️  CLAUDE PREVIEW VERSION
   This is the single-file version of App.jsx for Claude artifact preview only.

   The production version (in /src/App.jsx) imports from './lib/supabase' which
   the Claude artifact runtime can't resolve. Here, all Supabase helpers are
   replaced with no-op stubs so isSupabaseConfigured() returns false, the app
   falls back to dev mode (localStorage + seed data + role toggle).

   YOU CANNOT TEST FROM HERE:
   - Google sign-in
   - The pending-approval workflow
   - The whitelist
   - The audit log
   These all need a real Supabase backend. Use the production version
   (src/App.jsx) for that — deploy to Netlify with Supabase env vars set.

   YOU CAN TEST FROM HERE:
   - All UI flows for jutsus (browse, filter, add, edit, delete)
   - The "view slots" eye-icon button on Limited jutsus
   - Catalog management modal for bloodlines
   - The role toggle (Dev: User / Dev: Admin) to see admin-only UI
   - Filter chips, sorting, session list cart, personal tags
   ============================================================================ */

const isSupabaseConfigured        = () => false;
const fetchAllFromSupabase        = async () => null;
const upsertJutsu                 = async () => {};
const deleteJutsu                 = async () => {};
const upsertBloodline             = async () => {};
const deleteBloodline             = async () => {};
const saveSpecializationsToSupabase = async () => {};
const signInWithGoogle            = async () => { throw new Error('Sign-in not available in preview mode'); };
const signOut                     = async () => {};
const getCurrentSession           = async () => null;
const onAuthChange                = () => () => {};
const fetchMyProfile              = async () => null;
const fetchAllProfiles            = async () => [];
const setUserRole                 = async () => {};
const fetchWhitelist              = async () => [];
const addToWhitelist              = async () => {};
const removeFromWhitelist         = async () => {};
const fetchPendingJutsus          = async () => [];
const submitPendingJutsu          = async () => {};
const approvePendingJutsu         = async () => {};
const cancelPendingJutsu          = async () => {};
const buildJutsuPayload           = (j) => j;
const fetchRoleChangeLog          = async () => [];
...
function UserMenu({ profile, onSignIn, onSignOut, onOpenManagement, supabaseReady, devRole, onToggleDevRole }) {
  ...
  // Dev mode: no Supabase configured. Fall back to the original role toggle so
  // the UI is still testable without a backend.
  if (!supabaseReady) {
    return (
      <button onClick={onToggleDevRole}
              className={`text-xs px-3 py-1.5 font-bold rounded-lg border flex items-center gap-1.5 shrink-0 ${devRole === 'admin' ? 'bg-slate-700 border-slate-600' : 'bg-slate-800 border-slate-700'} text-white`}
              title="No Supabase env vars set — using dev-mode role toggle.">
        <Icon n={devRole === 'admin' ? 'Shield' : 'Key'} size={14}/>
        <span className="hidden sm:inline">Dev: {devRole === 'admin' ? 'Admin' : 'User'}</span>
      </button>
    );
  }
  ...
  return (
    <div className="relative shrink-0">
      <button onClick={() => setOpen(!open)}
        ...
        {profile.avatar_url ? (
          <img src={profile.avatar_url} alt="" className="w-6 h-6 rounded-md object-cover" referrerPolicy="no-referrer" />
        ) : (
          <div className="w-6 h-6 rounded-md bg-indigo-500 text-white text-xs font-bold flex items-center justify-center">
            {(profile.full_name || profile.email || '?').charAt(0).toUpperCase()}
          </div>
        )}
        <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ${roleColors[profile.role] || roleColors.user}`}>
          {profile.role}
        </span>
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-200 z-40 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center gap-3">
              {profile.avatar_url ? (
                <img src={profile.avatar_url} alt="" className="w-10 h-10 rounded-lg object-cover" referrerPolicy="no-referrer" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-slate-800 truncate">{profile.full_name || 'No name'}</div>
            ...
            {canManageUsers && (
              <button onClick={() => { setOpen(false); onOpenManagement(); }}
                ...
                <Icon n="Shield" size={14} className="text-amber-500"/> Manage Users{profile.role === 'owner' && ' & Whitelist'}
              </button>
            )}
      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full overflow-hidden max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="bg-slate-900 text-white p-5 flex justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-lg">Manage Users{isOwner && ' & Whitelist'}</h3>
          </div>
          <button onClick={onClose}><Icon n="X" size={18} /></button>
        </div>
  ...
  return (
    <div className="w-full min-h-screen bg-slate-200 flex flex-col font-sans text-slate-900">
      ...
      <div className="bg-slate-900 text-white p-4 sticky top-0 z-40 flex flex-col sm:flex-row justify-between items-center shadow-lg gap-3">
        ...
        <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto justify-center sm:justify-end scrollbar-hide pb-1 sm:pb-0">
          {isAdmin && (
