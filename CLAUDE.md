...
- Four `personal` fields in the sheet are computed, not typed in: **Submitted by** (`fetchProfileById(ownerId)`'s Discord username, live — not frozen at save time), **Character slot** (`ocSlotLabel()`, frozen into the sheet the first time it's saved so a later 4th OC doesn't retroactively relabel an earlier one), **Threat level** (`computeThreatLevel()` — averages the 4 stat ranks Chakra Level/Control, Speed, Strength on an F=E=0…S=5 scale, rounds half-down, maps back to a letter; blank until all four stats are set), and **Village / Shinobi rank / Clan-KKG**, whose *starting* value is auto-selected from `prefill` (only `PendingJutsuCard` can supply this — it's the OC's own pending submission data — and only ever applied once, to a brand-new sheet) but stays a normal editable field afterward. `ownerIdHint` (also only set by `PendingJutsuCard`, from `pending.submitted_by`) keeps "Submitted by" and a staff-created sheet's `owner_id` pointed at the actual player instead of whichever staff member is filling it in; a sheet started from the Roster page has no such hint and falls back to attributing itself to whoever clicks "create."

- The Techniques/PvE/Battle mode tables no longer take free-text jutsu names: `JutsuPicker` (inside `CharacterSheetModal.jsx`) searches the live jutsu catalog (`jutsus` prop — threaded down from `App.jsx`'s `db.jutsus` via `RosterPage`/`PendingJutsuCard`) and locks the row to the picked jutsu, auto-filling Rank/Nature when the jutsu only has one of each and narrowing the Choice to just that jutsu's own options when it has several. Battle mode slots are further filtered to Battlemode-type jutsus whose `bm_tier` matches that exact slot (Primary/Secondary/Tertiary).
