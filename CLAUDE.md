...
- The Techniques/Battle mode tables no longer take free-text jutsu names: `JutsuPicker` (inside `CharacterSheetModal.jsx`) searches the live jutsu catalog (`jutsus` prop — threaded down from `App.jsx`'s `db.jutsus` via `RosterPage`/`PendingJutsuCard`/`OCSubmissionModal`) and locks the row to the picked jutsu, auto-filling Rank/Nature when the jutsu only has one of each and narrowing the Choice to just that jutsu's own options when it has several. Battle mode slots are further filtered to Battlemode-type jutsus whose `bm_tier` matches that exact slot (Primary/Secondary/Tertiary). Each slot renders as a card (name/search box full width, Rank/Nature/Approved/Doc link stacked in a grid below) rather than a table row — the row-of-narrow-columns layout didn't leave the picker enough room. There is no PvE slots table any more (removed outright, including `LIMITS.pveSlots` and `techniques.pve` from the sheet shape).
